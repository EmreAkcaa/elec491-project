...
from utils import (
    ...
    load_te_edges, load_te_node_roles, load_te_matrix,
...
def _plot_network(
    ...
    for node in G.nodes():
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=8 + deg * 2, color=color, line=dict(width=0.5, color="#fff")),
            text=node, textposition="top center", textfont=dict(size=7),
            hovertemplate=f"<b>{node}</b><br>Sector: {sector}<br>Degree: {deg}<extra></extra>",
            showlegend=False,
        ))
...
def render_rmt(sector_map: dict):
    ...
    with st.container(border=True):
        ...
        with col_mst:
            ...
            if mst_choice == "Raw":
                fig = _plot_network(raw_edges, sector_map, edge_weight_col="distance")
            elif mst_choice == "Denoised":
                fig = _plot_network(denoised_edges, sector_map, edge_weight_col="distance")
            else:
                fig = _plot_network(denoised_edges, sector_map, edge_weight_col="distance")

            render_chart(fig, chart_id="rmt_mst", filename_base="rmt_mst",
                         title_key="rmt_mst",
                         default_title=f"MST Network ({mst_choice})")
...
def render_wavelets(sector_map: dict):
    ...
    with st.container(border=True):
        ...
        with col_mst:
            ...
            if not edges.empty:
                fig = _plot_network(edges, sector_map, edge_weight_col="distance")
                total_weight = edges["distance"].sum()
                render_chart(fig, chart_id=f"wav_mst_{scale_level}", filename_base="wavelet_mst",
                             title_key="wav_mst",
                             default_title=f"MST at {scale_label} (total weight: {total_weight:.1f})")
            else:
...
def render():
    ...
    sub_rmt, sub_glasso, sub_wavelet, sub_te = st.tabs([
        "RMT Denoising", "Graphical LASSO", "Wavelet Multi-Scale", "Transfer Entropy",
    ])
