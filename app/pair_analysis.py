...
Keep this module independent so it can be developed and tested separately.
"""
...
def render(
    ...
    """Render the full Pair Analysis view including sidebar controls."""

    # ------------------------------------------------------------------
    # Sidebar — ticker & window controls
    # ------------------------------------------------------------------
    st.sidebar.markdown("---")
    ...
    rc_window_type = st.sidebar.selectbox("Window type", ["rolling", "expanding", "ewm"], key="pa_rc_wtype")
    ...
    # ------------------------------------------------------------------
    # Event markers (independent "pa" key-prefix set)
    # ------------------------------------------------------------------
    ...
    st.caption(
        ...
        "**Spearman ρ** is rank-based and more robust to outliers and fat-tailed distributions."
    )
    ...
    st.caption(
        ...
        "Blue fill = positively correlated period; red fill = negatively correlated."
    )
    ...
    st.caption(
        "Both stocks rebased to **100** at the start of the window. "
        "The **shaded area between the two lines** highlights divergence — wider = decoupling."
    )
    ...
    if ticker_a in prices_window.columns and ticker_b in prices_window.columns:
        ...
        fig_price.add_trace(go.Scatter(x=pa.index, y=pa, name=ticker_a,
                                       mode="lines", line=dict(color="royalblue", width=2.2)))
        fig_price.add_trace(go.Scatter(x=pb.index, y=pb, name=ticker_b,
    ...
    # ------------------------------------------------------------------
    # Section 4 — Return scatter + OLS regression
    # ------------------------------------------------------------------
    st.markdown("---")
    st.subheader("Daily Return Scatter")
    st.caption(
        "Each point is one trading day: **(return of A, return of B)**. "
        "A tight diagonal cluster = high positive correlation. "
        "The **red regression line** slope is the OLS beta of B relative to A."
    )
    ...
    st.caption(
        "Overlaid histograms at 60% opacity so both distributions are visible. "
        "Wider spread = higher volatility; heavier left tail = more severe drawdown days."
    )
    ...
    st.caption(
        f"Rolling {rc_window}-day std of daily returns × √252. "
        "The **grey band** between the two lines shows when their volatility diverges. "
    ...
    if not both.empty:
        ...
        fig_vol.add_trace(go.Scatter(x=_vol_a.index, y=_vol_a, name=ticker_a,
                                     mode="lines", line=dict(color="royalblue", width=1.8)))
        fig_vol.add_trace(go.Scatter(x=_vol_b.index, y=_vol_b, name=ticker_b,
    ...
    st.caption(
        ...
        "Overlapping filled areas highlight periods of **joint drawdown** — "
        "stocks that fall together offer less diversification benefit."
    )
    ...
    # ------------------------------------------------------------------
    # Section 8 — MST placeholder
    # ------------------------------------------------------------------
    st.markdown("---")
    st.subheader("MST Network Position")
    st.info(
        f"**Coming soon.** This section will show where **{ticker_a}** and **{ticker_b}** sit "
        f"in the Minimum Spanning Tree — their nearest MST neighbors, the path length between them, "
        f"and whether they belong to the same cluster. "
        f"The `networkx` Cloud dependency is being resolved before this section is activated."
    )
