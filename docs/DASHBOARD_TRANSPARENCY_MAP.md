...
| **Base-currency sensitivity** | `cross_market.py:_render_bist_numeraire_section()` ~370+ | `data/bist_gold/` + `data/bist_usd/` + `data/results/numeraire_decomposition.json` + `docs/figures/numeraire_sector_shift.svg` | Three eigenvalue spectra (TRY/USD/Gold) + sector-decomposition SVG + interpretation paragraph | None |
